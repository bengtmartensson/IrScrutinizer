...
<xsl:stylesheet xmlns:girr="http://www.harctoolbox.org/Girr"
    ...
    <xsl:variable name="transformationVersion" select="'2025-02-04'" />
    ...
    <xsl:template match="/">
        ...
        <xsl:call-template name="define-no_sends"/>
...
//#define sendRaw_P sendRaw
    ...
    <xsl:template name="define-no_sends">
        <xsl:text>static unsigned int no_sends = 1U;
</xsl:text>
    </xsl:template>
...
void loop() {
    ...
    */
    ...
    <xsl:template match="girr:command" mode="case-line">
    ...
    <xsl:template match="girr:command" mode="using">
        ...
        <xsl:text>        IrSender.sendRaw_P(</xsl:text>
